...
export default { 
    ...
    async run( { guild, msg, author, args } ) { 
        ...
        if ( !target ) { 
            ...
            return await msg.reply( "Please specify a member to ban!" );

        }

        if ( !reason ) { 

            return await msg.reply( "Please specify a reason to ban this member!" );
        ...
        lobby.send( `${target.user.username} has been banned by ${author} for ${reason}.`);
